...
export default async (request: Request, response: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    const unassignTicket = dependencyContainer.get<UnassignTicket>(UnassignTicket);

    await unassignTicket.run(conversationId);

    response.status(HTTP_SUCCESS_CODES.OK).send('Ticket unassigned successfully.');
  } catch (error) {
    next(error);
  }
};
